var H=Object.defineProperty;var D=(l,e,n)=>e in l?H(l,e,{enumerable:!0,configurable:!0,writable:!0,value:n}):l[e]=n;var p=(l,e,n)=>D(l,typeof e!="symbol"?e+"":e,n);import{i as N,u as y,b as M,g as f}from"./index.js";class v{constructor(e){this.controller=e}render(){const e=document.createElement("section");e.className="view card",e.setAttribute("role","region"),e.setAttribute("aria-label","Home");const n=document.createElement("h2");n.textContent="Cerita Terbaru",e.appendChild(n);const t=document.createElement("div");return t.id="story-list",e.appendChild(t),this.controller.loadStories().then(async o=>{if(!o||o.length===0)t.textContent="Tidak ada data.";else for(const a of o){const i=document.createElement("article");i.className="card",i.tabIndex=0;const r=document.createElement("img");r.alt=a.photoUrl?`Foto cerita ${a.name}`:"";const d="/inter-final/public/assets/placeholder.png";r.src=a.photoUrl||d,r.width=200,r.height=120,i.appendChild(r);const c=document.createElement("div"),u=a.createdAt?new Date(a.createdAt).toLocaleDateString("id-ID",{year:"numeric",month:"long",day:"numeric"}):"-";c.innerHTML=`<h3>${a.name||"Unknown"}</h3><p>${a.description?a.description.slice(0,120):"-"}</p><p class="created-at">Dibuat: ${u}</p>`;const s=document.createElement("button");s.className="bookmark-btn",s.setAttribute("aria-label","Bookmark story");let m=await N(a.id).catch(()=>!1);s.textContent=m?"★ Bookmarked":"☆ Bookmark",m&&(s.style.color="#ffd700"),s.style.marginTop="8px",s.style.padding="4px 8px",s.style.cursor="pointer",s.addEventListener("click",async h=>{h.stopPropagation();try{m?(await y(a.id),m=!1,s.textContent="☆ Bookmark",s.style.color=""):(await M(a),m=!0,s.textContent="★ Bookmarked",s.style.color="#ffd700")}catch(g){console.error("Bookmark error:",g)}}),c.appendChild(s),i.appendChild(c),t.appendChild(i)}}).catch(o=>{t.textContent="Gagal memuat data."}),e}}p(v,"viewName","Home");class w{constructor(e){this.controller=e}render(){const e=document.createElement("section");e.className="view card",e.setAttribute("role","region"),e.setAttribute("aria-label","Peta Cerita");const n=document.createElement("h2");n.textContent="Peta Cerita Pengguna",e.appendChild(n);const t=document.createElement("div");return t.id="map",t.style.height="500px",t.style.width="100%",t.setAttribute("aria-label","Peta yang menampilkan lokasi story pengguna"),e.appendChild(t),setTimeout(async()=>{try{const o=await this.controller.loadStories();if(!window.L){t.innerHTML="<p>⚠️ Leaflet belum dimuat. Pastikan sudah menambahkan script Leaflet di index.html.</p>";return}const a=L.map("map").setView([-2.5,118],5);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap contributors"}).addTo(a);const i=[];(o||[]).forEach(r=>{var b,k;const d=parseFloat(r.lat||r.latitude||((b=r.location)==null?void 0:b.lat)),c=parseFloat(r.lon||r.longitude||((k=r.location)==null?void 0:k.lon));if(!isFinite(d)||!isFinite(c))return;const u=L.marker([d,c]).addTo(a),s=r.photoUrl||r.photo||"",m=r.name||r.username||"Anonim",h=r.createdAt?new Date(r.createdAt).toLocaleString("id-ID"):"Tanggal tidak diketahui",g=r.description||"-",B=r.id||r._id||"-",A=`
    ...
    `,setTimeout(()=>{location.hash="#/login"},2e3),e}}p(C,"viewName","Logout");class S{constructor(e){this.controller=e}render(){const e=document.createElement("section");e.className="view card",e.setAttribute("role","region"),e.setAttribute("aria-label","Bookmarked Stories");const n=document.createElement("h2");n.textContent="Story yang Disimpan",e.appendChild(n);const t=document.createElement("div");return t.id="bookmark-list",e.appendChild(t),this.loadBookmarks(t),e}async loadBookmarks(e){try{const n=await f();if(!n||n.length===0){e.innerHTML="<p>Tidak ada story yang disimpan. Bookmark story dari halaman Home untuk menyimpannya di sini.</p>";return}e.innerHTML="";for(const t of n){const o=document.createElement("article");o.className="card",o.tabIndex=0;const a=document.createElement("img");a.alt=t.photoUrl?`Foto cerita ${t.name}`:"";const i="/inter-final/public/assets/placeholder.png";a.src=t.photoUrl||i,a.width=200,a.height=120,a.onerror=()=>{a.src=i},o.appendChild(a);const r=document.createElement("div"),d=t.createdAt?new Date(t.createdAt).toLocaleDateString("id-ID",{year:"numeric",month:"long",day:"numeric"}):"-";r.innerHTML=`
          <h3>${t.name||"Unknown"}</h3>
          <p>${t.description?t.description.slice(0,120):"-"}</p>
          <p class="created-at">Dibuat: ${d}</p>
        `;const c=document.createElement("button");c.className="bookmark-btn",c.textContent="★ Hapus dari Bookmark",c.style.color="#ffd700",c.style.marginTop="8px",c.style.padding="4px 8px",c.style.cursor="pointer",c.setAttribute("aria-label","Hapus bookmark"),c.addEventListener("click",async u=>{u.stopPropagation();try{await y(t.id),o.remove(),(await f()).length===0&&(e.innerHTML="<p>Tidak ada story yang disimpan. Bookmark story dari halaman Home untuk menyimpannya di sini.</p>")}catch(s){console.error("Unbookmark error:",s),alert("Gagal menghapus bookmark")}}),r.appendChild(c),o.appendChild(r),e.appendChild(o)}}catch(n){console.error("Error loading bookmarks:",n),e.innerHTML="<p>Gagal memuat data bookmark.</p>"}}}p(S,"viewName","Bookmark");function P(l,e){function n(){const o=localStorage.getItem("token"),a=document.getElementById("nav-login"),i=document.getElementById("nav-register"),r=document.getElementById("nav-logout");o?(a&&(a.style.display="none"),i&&(i.style.display="none"),r&&(r.style.display="block")):(a&&(a.style.display="block"),i&&(i.style.display="block"),r&&(r.style.display="none"))}async function t(){const o=location.hash||"#/";let a=v;o.startsWith("#/map")?a=w:o.startsWith("#/add")?a=x:o.startsWith("#/login")?a=T:o.startsWith("#/register")?a=E:o.startsWith("#/logout")?a=C:o.startsWith("#/bookmark")&&(a=S);const i=new a(e);document.startViewTransition?document.startViewTransition(()=>{l.innerHTML="",l.appendChild(i.render())}):(l.classList.add("view-exit-active"),requestAnimationFrame(()=>{l.innerHTML="",l.appendChild(i.render()),l.classList.remove("view-exit-active")})),document.title=`Story Map - ${a.viewName}`,n()}return window.addEventListener("hashchange",t),window.addEventListener("load",()=>{location.hash||(location.hash="#/"),t()}),(document.readyState==="complete"||document.readyState==="interactive")&&(location.hash||(location.hash="#/"),t()),n(),{renderRoute:t}}export{P as default};
